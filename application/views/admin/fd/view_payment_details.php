                    ...
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody>
                            <?php foreach ($vw_rdpay as $ve => $vwrdp) { ?>
                                <?php $lst_id = $this->Admin_Fd_model->get_last_id($vwrdp['fd_id']); ?>
                                <tr>
                                    <td>
                                        <?php echo $ve + 1 ?>
                                    </td>
                                    ...
                                    <td>
                                        <?php if ($vwrdp['status'] == 1) { ?>
                                            ...
                                            <?php } else { ?>
                                                <a href="javascript:void(0);" style="width:30px;" class="btn-sm btn-primary"
                                                    ...
                                                    onclick="pay_fd_view(<?php echo $vwrdp['id'] ?>)" title='Click to Pay EMI'>
                                                    Pay Now
                                                </a>
                                            <?php } ?>
                                        <?php } else if ($vwrdp['status'] == 2) { ?>
                                                ...
                                                <a href="javascript:void(0);" data-toggle="modal" data-target="#view_paid_emi_modal"
                                                    onclick="view_paid_fd_payment(<?php echo $vwrdp['id'] ?>)"
    ...
    <div class="modal-dialog modal-md">
        <div class="modal-content">
            <div class="modal-body  card-primary card-outline">
                ...
                <div id="show_paid_fd_payment"></div>
            </div>
        </div>
        <!-- /.modal-content -->
    </div>
...
<script>
    ...
    function print_bill(id) {
        $.ajax({
            url: '<?= base_url() ?>' + 'admin/Fd/print_bill',
            type: "POST",
            data: {
                'id': id
            },
    ...
    function print_bound(id, user_id) {
        ...
        $.ajax({
            ...
            url: '<?= base_url() ?>' + 'admin/Fd/print_bound',
            type: "POST",
            data: {
                'id': id,
                'user_id': user_id
    ...
    function view_paid_fd_payment(id) {

        $.ajax({

            url: '<?= base_url() ?>' + 'admin/Fd/paid_fd_payment_view',
            type: "POST",
            data: {
                'id': id
            },
            success: function (data) {
                $('#show_paid_fd_payment').html(data);
            },
    ...
    function pay_fd_view(id) {

        $.ajax({

            url: '<?= base_url() ?>' + 'admin/Fd/view_fd_payment_pay',
            type: "POST",
            data: {
                'id': id
            },
    ...
    $('#pay').submit(function (e) {
        e.preventDefault();
        $.ajax({
            url: '<?= base_url() ?>admin/Fd/pay_fd_payment_data',
            type: "POST",
