...
<body class="hold-transition sidebar-mini layout-fixed ">
    <div class="wrapper">
        ...
        <div class="modal fade" id="view_rd_payment_modal">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-body card-primary card-outline">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        ...
                        <h4 class="modal-title">RD Payment Data Details</h4>
                        <div id="show_rd_payment"></div>
                    </div>
                </div>
                <!-- /.modal-content -->
            </div>
        ...
        <div class="modal fade" id="pay_emi_modal">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title">Close RD Plan</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <form id="close_rd_plan_data" method="post" accept-charset="utf-8" enctype="multipart/form-data">
                        <div class="modal-body">
                            <div id="show_close_rd"></div>
                        </div>
                        <div class="modal-footer justify-content-between">
                            <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
    ...
    function print_member_details(id, user_id) {
        $.ajax({
            url: '<?= base_url() ?>' + 'admin/Rd/print_member_details',
            type: "POST",
            data: {
                'id': id,
                'user_id': user_id,
    ...
    function popup(data) {
        ...
        function view_rd_payment_details(id) {
            $.ajax({
                url: '<?= base_url() ?>' + 'admin/Rd/rd_payment_details',
                type: "POST",
                data: {
                    'id': id
                },
                success: function (data) {
                    $('#show_rd_payment').html(data);
                },
            });
        }
